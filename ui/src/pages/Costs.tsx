...
import { SubscriptionBudgetCard } from "../components/SubscriptionBudgetCard";
...
export function Costs() {
  ...
  const { data: quotaData, isLoading: quotaLoading } = useQuery({
    ...
    enabled: !!selectedCompanyId && (mainTab === "providers" || mainTab === "budgets" || mainTab === "overview"),
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            ...
            <MetricTile
              label="Budget"
              value={(() => {
                const anthropicWindows = (quotaWindowsByProvider.get("anthropic") ?? []).filter(
                  (w) => w.label.toLowerCase().replace(/[^a-z0-9]+/g, "") !== "extrausage",
                );
                const maxPct = anthropicWindows.reduce((max, w) => Math.max(max, w.usedPercent ?? 0), 0);
              ...
              subtitle={(() => {
                ...
                const peakWindow = anthropicWindows.reduce<QuotaWindow | null>(
                  (max, w) => ((w.usedPercent ?? 0) > (max?.usedPercent ?? 0) ? w : max), null,
                );
                if (peakWindow) return `Subscription · ${peakWindow.label}`;
                return "No subscription quota available";
              })()}
              icon={Coins}
            />
            <MetricTile
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        ...
        <TabsContent value="budgets" className="mt-4 space-y-4">
          <SubscriptionBudgetCard
            quotaWindows={quotaWindowsByProvider.get("anthropic") ?? []}
            source={quotaSourcesByProvider.get("anthropic") ?? null}
            error={quotaErrorsByProvider.get("anthropic") ?? null}
            loading={quotaLoading}
          />
          {!quotaLoading && (quotaWindowsByProvider.get("anthropic")?.length ?? 0) === 0 && !quotaErrorsByProvider.get("anthropic") ? (
            <Card>
              <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                No subscription quota data available. Log in to Claude via <code className="text-foreground">claude login</code> to enable quota tracking.
              </CardContent>
            </Card>
          ) : null}
        </TabsContent>
