...
const ALL_ISSUE_STATUSES = ["backlog", "todo", "in_progress", "in_review", "blocked", "done", "cancelled", "deferred"];
...
export function issueService(db: Db) {
  ...
  return {
    ...
    checkout: async (id: string, agentId: string, expectedStatuses: string[], checkoutRunId: string | null) => {
      ...
      // If a queued (never-started) executionRun exists for the same agent, cancel it so checkout can proceed
      if (checkoutRunId) {
        const blocking = await db
          .select({ id: issues.executionRunId })
          .from(issues)
          .where(and(eq(issues.id, id), eq(issues.assigneeAgentId, agentId)))
          .then((rows) => rows[0]?.id ?? null);
        if (blocking && blocking !== checkoutRunId) {
          const cancelled = await db
            .update(heartbeatRuns)
            .set({ status: "cancelled", finishedAt: now, updatedAt: now })
            .where(and(eq(heartbeatRuns.id, blocking), inArray(heartbeatRuns.status, ["queued"])))
            .returning()
            .then((rows) => rows[0] ?? null);
          if (cancelled) {
            await db
              ...
              .where(and(eq(issues.id, id), eq(issues.executionRunId, blocking)));
          }
        }
      }
    ...
    release: async (id: string, actorAgentId?: string, actorRunId?: string | null) => {
      ...
      // Cancel any queued runs blocking this issue before releasing
      if (existing.executionRunId) {
        await db
          .update(heartbeatRuns)
          .set({ status: "cancelled", finishedAt: new Date(), updatedAt: new Date() })
          .where(and(eq(heartbeatRuns.id, existing.executionRunId), inArray(heartbeatRuns.status, ["queued"])));
      }
