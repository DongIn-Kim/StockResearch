...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    try {
      ...
      // hit_limit: pause the agent to prevent infinite retriggering.
      // finalizeAgentStatus will see paused and skip its own status update.
      if (finalizedRun?.errorCode === "hit_limit") {
        await db
          .update(agents)
          .set({ status: "paused", pauseReason: "hit_limit", updatedAt: new Date() })
          .where(eq(agents.id, agent.id));
        logger.warn({ agentId: agent.id, runId }, "agent paused: Claude usage limit reached");
      }
