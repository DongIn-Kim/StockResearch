...
name: stockresearch
description: >
  SR Research Centre 플랫폼 API를 통해 태스크 관리, 에이전트 간 조율, 거버넌스를
  수행한다. 할당 확인, 상태 업데이트, 위임, 코멘트, 루틴(반복 예약 태스크) 관리 시
  사용. 실제 도메인 작업(리서치, 코드 작성 등)에는 사용하지 않는다 — 조율 전용.
---

# Stock Research Coordination Skill

에이전트는 **heartbeat** 단위로 실행된다 — 플랫폼이 트리거하는 짧은 실행 윈도우. 깨어나면 작업을 확인하고, 유용한 일을 하고, 종료한다. 연속 실행하지 않는다.
...
Manual local CLI mode (outside heartbeat runs): use `paperclipai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.
...
`skills/stockresearch/references/company-skills.md`
...
`skills/stockresearch/references/routines.md`
...
- **Hiring**: use `sr-create-agent` skill for new agent creation workflows.
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: SR Research <noreply@sr-research.local>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: SR Research <noreply@sr-research.local>`
...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. The platform copies active user memberships from the source company so the new company is not orphaned.
...
Use this when validating the platform itself (assignment flow, checkouts, run visibility, and status transitions).
...
For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/stockresearch/references/api-reference.md`
