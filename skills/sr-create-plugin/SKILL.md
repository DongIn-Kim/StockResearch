---
name: sr-create-plugin
description: >
  현재 alpha SDK/런타임으로 플러그인 생성. 플러그인 패키지 스캐폴딩, 예제 플러그인
  추가, 플러그인 작성 문서 업데이트 시 사용. worker/UI surface, 라우트 규칙,
  스캐폴드 절차, 검증 단계를 다룬다.
---
...
For a plugin that lives outside this repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.paperclip-sdk/`:
...
- is installed from an absolute local path during development
